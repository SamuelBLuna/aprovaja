...
export default function QuestoesAluno() {
 ...
 return (
 <div>
 <p className="text-slate-500 mb-4">Nenhuma questão encontrada para esse filtro.</p>
 ...
 <button onClick={() => setPraticando(false)} className="flex items-center gap-1 text-slate-500 hover:text-ink text-sm transition-colors"><ArrowLeft className="w-3.5 h-3.5" /> Trocar filtro</button>
 <span className="text-slate-500 text-sm">{indice + 1} de {questoes.length}</span>
 </div>
 ...
 <button disabled={indice === 0} onClick={() => setIndice((i) => i - 1)} className="flex items-center gap-1 text-sm text-slate-500 disabled:opacity-30 hover:text-ink transition-colors"><ArrowLeft className="w-3.5 h-3.5" /> Anterior</button>
 ...
 <select value={materiaId} onChange={(e) => setMateriaId(e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm bg-white focus:border-gold outline-none transition-colors">
 <option value="">Selecione a matéria</option>
 {materias.map((m) => <option key={m.id} value={m.id}>{m.nome}</option>)}
 </select>
 <select value={topicoId} onChange={(e) => setTopicoId(e.target.value)} disabled={!materiaId} className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm bg-white focus:border-gold outline-none transition-colors disabled:opacity-50">
 <option value="">Todos os tópicos</option>
 {topicos.map((t) => <option key={t.id} value={t.id}>{t.nome}</option>)}
 </select>
 <button onClick={iniciar} disabled={!materiaId} className="w-full bg-ink text-white py-2.5 rounded-lg text-sm font-medium hover:bg-ink-light transition-colors disabled:opacity-40">
 Começar a praticar
 </button>
 </div>
 </div>
