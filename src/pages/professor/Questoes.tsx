...
export default function Questoes() {
 ...
 actions={modo === 'lista' ? (
 <>
 <button onClick={() => setModo('novo_grupo')} className="flex items-center gap-1.5 border border-slate-200 text-ink px-3 py-2 rounded-lg text-sm hover:bg-slate-100 transition-colors bg-white">
 <Layers className="w-3.5 h-3.5" /> Grupo de questões
 </button>
 <button onClick={iniciarNovaAvulsa} className="flex items-center gap-1.5 bg-ink text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-ink-light transition-colors ">
 ...
 <p className="text-sm text-slate-500 mb-4">{editandoAvulsaId ? 'Editando questão avulsa' : 'Nova questão avulsa — sem vínculo com nenhum grupo.'}</p>
 <div className="grid grid-cols-2 gap-3 mb-4">
 <select required value={materiaId} onChange={(e) => setMateriaId(e.target.value)} className="border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white">
 <option value="">Matéria *</option>
 {materias.map((m) => <option key={m.id} value={m.id}>{m.nome}</option>)}
 </select>
 <select value={topicoId} onChange={(e) => setTopicoId(e.target.value)} disabled={!materiaId} className="border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white">
 ...
 {modo === 'novo_grupo' && (
 <form onSubmit={criarGrupo} className="bg-white border border-slate-200 rounded-xl p-5 mb-6 space-y-3">
 <p className="text-sm text-slate-500">Use um grupo quando várias perguntas dependem do mesmo texto — ex: interpretação de texto ou estudo de caso. Depois de criar, você adiciona as perguntas uma a uma.</p>
 ...
 className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:border-gold" />
 <div className="grid grid-cols-2 gap-3">
 <select required value={grupoMateriaId} onChange={(e) => setGrupoMateriaId(e.target.value)} className="border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white">
 <option value="">Matéria *</option>
 {materias.map((m) => <option key={m.id} value={m.id}>{m.nome}</option>)}
 </select>
 <select value={grupoTopicoId} onChange={(e) => setGrupoTopicoId(e.target.value)} disabled={!grupoMateriaId} className="border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white">
 ...
 <button className="bg-ink text-white px-4 py-2.5 rounded-lg text-sm hover:bg-ink-light transition-colors ">Criar grupo e adicionar perguntas</button>
 <button type="button" onClick={() => setModo('lista')} className="text-slate-500 text-sm hover:underline">Cancelar</button>
 </div>
 </form>
 )}
 ...
 <>
 {/* GRUPOS */}
 <h2 className="font-serif text-lg text-ink mb-3 flex items-center gap-2"><Layers className="w-4 h-4 text-slate-300" /> Grupos de questões</h2>
 ...
 <p className="text-slate-400 text-xs">{g.materias?.nome}{g.topicos?.nome ? ` — ${g.topicos.nome}` : ''} · {perguntas.length || '…'} pergunta(s)</p>
 </div>
 <div className="flex items-center gap-3 shrink-0">
 <button onClick={(e) => { e.stopPropagation(); excluirGrupo(g.id) }} className="text-slate-300 hover:text-erro transition-colors"><Trash2 className="w-4 h-4" /></button>
 <span className={`text-slate-300 transition-transform ${aberto ? 'rotate-90' : ''}`}>›</span>
 </div>
 </div>

 {aberto && (
 <div className="border-t border-slate-200 px-4 py-4 bg-paper/60 space-y-4">
 {editandoInfo ? (
 ...
 className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:border-gold" />
 <div className="grid grid-cols-2 gap-3">
 <select value={editGrupoMateriaId} onChange={(e) => setEditGrupoMateriaId(e.target.value)} className="border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white">
 <option value="">Matéria *</option>
 {materias.map((m) => <option key={m.id} value={m.id}>{m.nome}</option>)}
 </select>
 <select value={editGrupoTopicoId} onChange={(e) => setEditGrupoTopicoId(e.target.value)} disabled={!editGrupoMateriaId} className="border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white">
 <option value="">Tópico (opcional)</option>
 ...
 className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:border-gold" />
 <div className="flex gap-2">
 <button onClick={() => salvarEdicaoInfoGrupo(g.id)} className="bg-ink text-white px-4 py-2.5 rounded-lg text-sm hover:bg-ink-light transition-colors ">Salvar alterações</button>
 <button onClick={() => setEditandoInfoGrupoId(null)} className="text-slate-500 text-sm hover:underline">Cancelar</button>
 </div>
 </div>
 ) : (
 <div className="bg-white border border-slate-200 rounded-xl p-3">
 <p className="text-sm text-slate-600 whitespace-pre-wrap mb-2">{g.texto_base}</p>
 ...
 <div key={q.id} className="bg-white border border-slate-200 rounded-xl px-4 py-3 flex justify-between items-start">
 <div>
 <div className="flex flex-wrap items-center gap-2 mb-1 text-xs">
 <span className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded">{q.tipo === 'multipla_escolha' ? 'Múltipla escolha' : 'V ou F'}</span>
 ...
 <div className="bg-white border border-dashed border-slate-300 rounded p-4">
 ...
 {grupos.length === 0 && <p className="text-slate-500 text-sm">Nenhum grupo criado ainda.</p>}
 </div>

 {/* AVULSAS */}
 <div className="flex flex-wrap items-center gap-3 mb-3">
 <h2 className="text-base font-semibold text-slate-900">Questões avulsas</h2>
 <label className="flex items-center gap-1.5 text-sm text-slate-500 ml-auto cursor-pointer">
 ...
 <select value={filtroMateria} onChange={(e) => setFiltroMateria(e.target.value)} className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm bg-white">
 ...
 {avulsas.map((q) => (
 <div key={q.id} className="bg-white border border-slate-200 rounded-xl px-4 py-3.5 flex items-start justify-between gap-4">
 <div className="flex-1 min-w-0">
 <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
 <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full text-[11px] font-medium">{q.materias?.nome}</span>
 {q.topicos?.nome && <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full text-[11px] font-medium">{q.topicos.nome}</span>}
 <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full text-[11px] font-medium">{q.tipo === 'multipla_escolha' ? 'Múltipla escolha' : 'V ou F'}</span>
 ...
 <button onClick={() => iniciarEdicaoAvulsa(q)} title="Editar" className="p-1.5 text-slate-300 hover:text-gold transition-colors"><Pencil className="w-4 h-4" /></button>
 <button onClick={() => alternarRevisaoAvulsa(q.id, q.precisa_revisao)} title={q.precisa_revisao ? 'Marcar como revisada' : 'Marcar p/ revisão'} className={`p-1.5 transition-colors ${q.precisa_revisao ? 'text-erro' : 'text-slate-300 hover:text-erro'}`}>
 {q.precisa_revisao ? <RotateCcw className="w-4 h-4" /> : <AlertTriangle className="w-4 h-4" />}
 </button>
 <button onClick={() => excluirAvulsa(q.id)} title="Excluir" className="p-1.5 text-slate-300 hover:text-erro transition-colors"><Trash2 className="w-4 h-4" /></button>
 </div>
 </div>
 ))}
 {avulsas.length === 0 && <p className="text-slate-500 text-sm">Nenhuma questão encontrada.</p>}
 </div>
 </>
 )}
 </div>
