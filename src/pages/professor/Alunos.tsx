    ...
    return (
      <div>
        ...
        <div className="bg-white border border-ink/[0.07] rounded-xl shadow-soft p-10 text-center">
          <p className="text-ink/60 text-sm">Você ainda não tem nenhuma turma.</p>
        </div>
      </div>
    )
  }
              ...
              <div key={a.aluno_id} className="bg-white border border-ink/[0.07] rounded-xl shadow-soft overflow-hidden">
                <div className="flex items-center justify-between gap-3 px-4 py-3.5 cursor-pointer hover:bg-ink/[0.015] transition-colors" onClick={() => abrirDetalhe(a.aluno_id)}>
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-9 h-9 rounded-full bg-ink/5 text-ink/60 flex items-center justify-center text-xs font-semibold shrink-0">
                      {iniciais(a.nome)}
                    </div>
                  ...
                  <div className="flex items-center gap-6 shrink-0">
                    <div className="text-right hidden sm:block">
                      <p className="text-ink/40 text-[11px] uppercase tracking-wide">Questões</p>
                      <p className="text-ink text-sm font-medium">{a.total}</p>
                    </div>
                    <div className="text-right hidden sm:block">
                      <p className="text-ink/40 text-[11px] uppercase tracking-wide">Acerto</p>
                      <p className={`text-sm font-medium ${a.taxa >= 70 ? 'text-acerto' : a.taxa >= 50 ? 'text-gold-dark' : a.total > 0 ? 'text-erro' : 'text-ink/40'}`}>
                        {a.total > 0 ? `${a.taxa}%` : '—'}
                      </p>
                    </div>
                    <div className="text-right hidden md:block">
                      <p className="text-ink/40 text-[11px] uppercase tracking-wide">Última atividade</p>
                      <p className="text-ink text-sm">{a.ultimaAtividade ? new Date(a.ultimaAtividade).toLocaleDateString('pt-BR') : 'Nenhuma'}</p>
                    </div>
                    {aberto ? <ChevronDown className="w-4 h-4 text-ink/30" /> : <ChevronRight className="w-4 h-4 text-ink/30" />}
                  </div>
                </div>

                {aberto && (
                  <div className="border-t border-ink/[0.06] px-4 py-4 bg-paper/60">
                        ...
                        <div className="bg-white border border-ink/[0.07] rounded-xl shadow-soft p-4">
                          {materiaSelecionada ? (
                            <>
                              ...
                              {topicos.length === 0 ? (
                                <p className="text-ink/40 text-sm">Nenhum tópico registrado.</p>
                              ) : (
                            ...
                            <>
                              ...
                              <p className="text-xs text-ink/40 mb-3">Clique numa barra pra ver por tópico.</p>
                              {materias.length === 0 ? (
