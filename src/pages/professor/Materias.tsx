...
export default function Materias() {
 ...
 className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white focus:border-gold"
 />
 <button className="bg-ink text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-ink-light transition-colors ">Adicionar</button>
 </form>
 ...
 <button onClick={() => setEditandoMateriaId(null)} className="text-slate-500 text-xs hover:underline">Cancelar</button>
 </div>
 ) : (
 ...
 <span className="text-slate-400 text-xs">{topicos.length} tópico{topicos.length !== 1 ? 's' : ''}</span>
 </div>
 )}
 {!editandoEssaMateria && (
 ...
 <span className="text-slate-400 cursor-pointer" onClick={() => setExpandida(aberta ? null : m.id)}>{aberta ? '▾' : '▸'}</span>
 </div>
 )}
 </div>

 {aberta && (
 <div className="border-t border-slate-200 px-4 py-3 bg-paper/50">
 <ul className="space-y-1.5 mb-3">
 {topicos.map((t) => {
 const editandoEsseTopico = editandoTopicoId === t.id
 return (
 <li key={t.id} className="text-sm bg-white border border-slate-200 rounded px-3 py-2">
 {editandoEsseTopico ? (
 <div className="flex flex-col sm:flex-row gap-2">
 <input value={editTopicoNome} onChange={(e) => setEditTopicoNome(e.target.value)} autoFocus
 className="flex-1 border border-slate-200 rounded-lg px-2 py-1 text-sm" placeholder="Nome" />
 <input value={editTopicoDescricao} onChange={(e) => setEditTopicoDescricao(e.target.value)}
 className="flex-1 border border-slate-200 rounded-lg px-2 py-1 text-sm" placeholder="Detalhe" />
 <div className="flex gap-2 shrink-0">
 <button onClick={() => salvarEdicaoTopico(m.id, t.id)} className="text-acerto text-xs hover:underline">Salvar</button>
 <button onClick={() => setEditandoTopicoId(null)} className="text-slate-500 text-xs hover:underline">Cancelar</button>
 </div>
 </div>
 ) : (
 <div className="flex items-center justify-between">
 <span className="text-slate-700">{t.nome}{t.descricao ? <span className="text-slate-500"> — {t.descricao}</span> : null}</span>
 ...
 {topicos.length === 0 && <p className="text-slate-400 text-sm">Nenhum tópico ainda.</p>}
 </ul>
 ...
 className="flex-1 border border-slate-200 rounded-lg px-3 py-1.5 text-sm bg-white focus:border-gold"
 />
 <input
 value={novaDescricao[m.id] || ''}
 onChange={(e) => setNovaDescricao((prev) => ({ ...prev, [m.id]: e.target.value }))}
 placeholder="Detalhe (ex: arts. 319 a 321)"
 className="sm:w-56 border border-slate-200 rounded-lg px-3 py-1.5 text-sm bg-white focus:border-gold"
 />
 ...
 {materias.length === 0 && <p className="text-slate-500 text-sm">Nenhuma matéria criada ainda.</p>}
 </div>
 </div>
 )
}
