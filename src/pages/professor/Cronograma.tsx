...
export default function Cronograma() {
 ...
 actions={
 <button onClick={() => setShowNovaTurma((v) => !v)} className="flex items-center gap-1.5 border border-slate-200 text-ink px-3 py-2 rounded-lg text-sm hover:bg-slate-100 transition-colors bg-white">
 ...
 {showNovaTurma && (
 <form onSubmit={criarTurma} className="bg-white border border-slate-200 rounded-xl p-5 mb-6 grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
 <div className="sm:col-span-2">
 <label className="block text-xs text-slate-500 mb-1">Nome da turma</label>
 ...
 <div>
 <label className="block text-xs text-slate-500 mb-1">Fim (data da prova)</label>
 <input required type="date" value={dataFimTurma} onChange={(e) => setDataFimTurma(e.target.value)}
 className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm" />
 </div>
 <button className="sm:col-span-4 bg-ink text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-ink-light transition-colors ">Criar turma</button>
 </form>
 )}

 {turmas.length === 0 ? (
 <p className="text-slate-500 text-sm">Crie sua primeira turma para montar o cronograma.</p>
 ) : (
 <>
 <div className="flex flex-wrap items-center gap-3 mb-3 bg-white border border-slate-200 rounded-xl px-4 py-3">
 <select value={turmaId} onChange={(e) => setTurmaId(e.target.value)} className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm bg-white">
 {turmas.map((t) => <option key={t.id} value={t.id}>{t.nome}{t.status === 'encerrada' ? ' (encerrada)' : ''}</option>)}
 </select>
 {turmaAtual && (
 <>
 <span className="text-sm text-slate-500">Código: <strong className="text-ink font-mono tracking-wide">{turmaAtual.codigo_turma}</strong></span>
 <span className="text-sm text-slate-500 hidden sm:inline">{turmaAtual.data_inicio} até {turmaAtual.data_fim}</span>
 ...
 {turmaAtual?.status === 'ativa' ? (
 <button onClick={abrirNovo} className="flex items-center gap-1.5 bg-ink text-white px-3 py-1.5 rounded-lg text-sm hover:bg-ink-light transition-colors ">
 <Plus className="w-3.5 h-3.5" /> Adicionar atividade
 </button>
 ) : (
 <span className="text-xs text-slate-400 italic">Turma encerrada — sem novas atividades</span>
 )}
 </div>

 {showForm && (
 <form onSubmit={salvarItem} className="bg-white border border-slate-200 rounded-xl p-4 mb-4 space-y-3">
 ...
 <p className="text-xs text-slate-500 mb-2">Questões vinculadas a esta atividade (opcional):</p>

 {questoesSelecionadas.size > 0 && (
 <div className="bg-paper/60 border border-slate-200 rounded p-2 mb-2 max-h-32 overflow-y-auto space-y-1">
 {Array.from(questoesSelecionadas.values()).map((q) => (
 <div key={q.id} className="flex items-start justify-between gap-2 text-xs bg-white border border-slate-200 rounded px-2 py-1.5">
 <span className="text-slate-700">{q.enunciado.slice(0, 90)}{q.enunciado.length > 90 ? '…' : ''}</span>
 ...
 <button type="button" onClick={() => setModoSelecao('manual')} className={modoSelecao === 'manual' ? 'text-ink font-medium underline' : 'text-slate-500'}>Escolher manualmente</button>
 <button type="button" onClick={() => setModoSelecao('aleatorio')} className={modoSelecao === 'aleatorio' ? 'text-ink font-medium underline' : 'text-slate-500'}>Sortear questões</button>
 </div>
 ...
 {questoesDisponiveis.length === 0 && <p className="text-slate-400 text-xs">Nenhuma questão nesta matéria ainda.</p>}
 </div>
 ) : (
 <div className="flex flex-wrap items-end gap-2">
 <select value={topicoAleatorio} onChange={(e) => setTopicoAleatorio(e.target.value)} className="border border-slate-200 rounded-lg px-2 py-1.5 text-xs bg-white">
 ...
 <button type="button" onClick={sortear} className="border border-slate-200 rounded-lg px-3 py-1.5 text-xs hover:bg-slate-100">Sortear</button>
 </div>
 )}
 </div>
 )}
 ...
 <button className="bg-ink text-white px-4 py-2.5 rounded-lg text-sm hover:bg-ink-light transition-colors ">{editandoId ? 'Salvar alterações' : 'Salvar no cronograma'}</button>
 <button type="button" onClick={() => { setShowForm(false); resetForm() }} className="text-slate-500 text-sm hover:underline">Cancelar</button>
 </div>
 </form>
 )}
 ...
 {itensDoDia.map((item) => (
 <div key={item.id} className="bg-white border border-slate-200 rounded-xl px-4 py-3 flex justify-between items-start gap-3">
 <div className="min-w-0">
 ...
 {item.descricao && <p className="text-slate-500 text-sm mt-1 ml-4">{item.descricao}</p>}
 {item.data_inicio !== item.data_fim && (
 <p className="text-slate-400 text-xs mt-1 ml-4">Período: {item.data_inicio} até {item.data_fim}</p>
 )}
 </div>
 <div className="flex items-center gap-1 shrink-0 ml-3">
 <button onClick={() => abrirEdicao(item)} title="Editar" className="p-1.5 text-slate-300 hover:text-gold transition-colors"><Settings2 className="w-4 h-4" /></button>
 <button onClick={() => excluirItem(item.id)} title="Remover" className="p-1.5 text-slate-300 hover:text-erro transition-colors">✕</button>
 </div>
 </div>
 ))}
 {itensDoDia.length === 0 && (
 <div className="text-center py-10 text-slate-300 text-sm border border-dashed border-slate-200 rounded-xl">Nada planejado para este dia ainda.</div>
 )}
 </div>
 </div>
 </div>
