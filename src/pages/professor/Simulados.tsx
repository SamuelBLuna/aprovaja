...
import { Plus, BarChart3, Pencil, Trash2, ClipboardList } from 'lucide-react'
...
export default function Simulados() {
 ...
 <button onClick={() => (showForm ? (setShowForm(false), resetForm()) : abrirNovo())} className="flex items-center gap-1.5 bg-ink text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-ink-light transition-colors ">
 ...
 {showForm && (
 <form onSubmit={salvarSimulado} className="bg-white border border-slate-200 rounded-xl p-5 mb-6 space-y-4">
 ...
 <input required value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Título do simulado" className="sm:col-span-2 border border-slate-200 rounded-lg px-3 py-2 text-sm focus:border-gold" />
 <input type="number" min={5} value={tempoLimite} onChange={(e) => setTempoLimite(Number(e.target.value))} className="border border-slate-200 rounded-lg px-3 py-2 text-sm" placeholder="Minutos" />
 </div>
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
 <input value={descricao} onChange={(e) => setDescricao(e.target.value)} placeholder="Descrição (opcional)" className="border border-slate-200 rounded-lg px-3 py-2 text-sm focus:border-gold" />
 <div>
 <label className="text-xs text-slate-500 mr-2">Disponível até (opcional):</label>
 <input type="datetime-local" value={dataLimite} onChange={(e) => setDataLimite(e.target.value)} className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm" />
 </div>
 </div>

 {materiasComSelecao.length > 0 && (
 <div className="bg-paper/60 border border-slate-200 rounded p-3">
 ...
 return (
 <div key={mid} className="bg-white border border-slate-200 rounded">
 <div className="flex items-center justify-between px-3 py-2">
 <span className="text-sm text-ink font-medium">{i + 1}. {nomeDaMateria(mid)} <span className="text-slate-400 font-normal">({questoesDaMateria.length} questões)</span></span>
 <div className="flex items-center gap-1 shrink-0">
 <button type="button" onClick={() => moverMateria(i, -1)} disabled={i === 0} className="w-6 h-6 rounded hover:bg-slate-100 disabled:opacity-20 text-slate-500">↑</button>
 <button type="button" onClick={() => moverMateria(i, 1)} disabled={i === materiasComSelecao.length - 1} className="w-6 h-6 rounded hover:bg-slate-100 disabled:opacity-20 text-slate-500">↓</button>
 </div>
 </div>
 <div className="border-t border-slate-200 px-3 py-2 space-y-1 max-h-28 overflow-y-auto">
 {questoesDaMateria.map((q) => (
 <div key={q.id} className="flex items-start justify-between gap-2 text-xs">
 <span className="text-slate-600">{q.enunciado.slice(0, 80)}{q.enunciado.length > 80 ? '…' : ''}</span>
 ...
 <p className="text-xs text-slate-400 mt-2">As setas mudam a ordem em que as matérias aparecem pro aluno — útil pra seguir a ordem oficial do edital.</p>
 </div>
 )}

 <div className="flex gap-4 border-b border-slate-200 pb-2">
 <button type="button" onClick={() => setModo('manual')} className={`text-sm ${modo === 'manual' ? 'text-ink font-medium border-b-2 border-gold' : 'text-slate-500'}`}>Escolher manualmente</button>
 <button type="button" onClick={() => setModo('aleatorio')} className={`text-sm ${modo === 'aleatorio' ? 'text-ink font-medium border-b-2 border-gold' : 'text-slate-500'}`}>Sortear questões</button>
 </div>

 <select value={materiaId} onChange={(e) => setMateriaId(e.target.value)} className="border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white">
 ...
 {modo === 'manual' ? (
 materiaId && (
 <div className="max-h-56 overflow-y-auto border border-slate-200 rounded p-2 space-y-1">
 ...
 <span className="text-slate-700">{q.enunciado.slice(0, 110)}{q.enunciado.length > 110 ? '…' : ''}</span>
 </label>
 ))}
 {questoesFiltradas.length === 0 && <p className="text-slate-400 text-sm">Nenhuma questão nesta matéria.</p>}
 </div>
 )
 ) : (
 materiaId && (
 ...
 <div>
 <label className="block text-xs text-slate-500 mb-1">Tópico (opcional)</label>
 <select value={topicoAleatorio} onChange={(e) => setTopicoAleatorio(e.target.value)} className="border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white">
 ...
 <input type="number" min={1} value={quantidade} onChange={(e) => setQuantidade(Number(e.target.value))} className="w-24 border border-slate-200 rounded-lg px-3 py-2 text-sm" />
 </div>
 <button type="button" onClick={sortear} className="border border-slate-200 rounded-lg px-3 py-2 text-sm hover:bg-slate-100">Sortear</button>
 </div>
 )
 )}

 <button className="bg-ink text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-ink-light transition-colors ">{editandoId ? 'Salvar alterações' : 'Criar simulado'}</button>
 </form>
 )}
 ...
 <p className="text-slate-500 text-xs">{s.tempo_limite_minutos} min {s.data_limite ? `· disponível até ${new Date(s.data_limite).toLocaleString('pt-BR')}` : ''}</p>
 </div>
 <div className="flex items-center gap-1 shrink-0">
 <button onClick={() => abrirResultados(s.id)} title="Ver resultados" className={`p-1.5 transition-colors ${aberto ? 'text-gold' : 'text-slate-300 hover:text-gold'}`}><BarChart3 className="w-4 h-4" /></button>
 <button onClick={() => abrirEdicao(s)} title="Editar" className="p-1.5 text-slate-300 hover:text-gold transition-colors"><Pencil className="w-4 h-4" /></button>
 <button onClick={() => excluirSimulado(s.id)} title="Excluir" className="p-1.5 text-slate-300 hover:text-erro transition-colors"><Trash2 className="w-4 h-4" /></button>
 </div>
 </div>

 {aberto && (
 <div className="border-t border-slate-200 bg-paper/50 px-4 py-4">
 {finalizados.length === 0 ? (
 <p className="text-slate-400 text-sm">Nenhum aluno finalizou esse simulado ainda.</p>
 ) : (
 <div className="space-y-2">
 {finalizados.map((t) => {
 ...
 return (
 <div key={t.id} className="bg-white border border-slate-200 rounded">
 ...
 <p className="text-xs text-slate-400">{formatarTempo(t.tempo_total_segundos || 0)}</p>
 </div>
 ...
 <span className="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded">{r.questoes.materias?.nome}</span>
 ...
 <p className="text-slate-500 mt-1">Resposta: <strong>{r.resposta_dada}</strong>{!r.correta && <> · Correta: <strong>{r.questoes.resposta_correta}</strong></>}</p>
 </div>
 ))}
 {!detalhesPorTentativa[t.id] && <p className="text-slate-400 text-xs">Carregando…</p>}
 </div>
 )}
 </div>
 )
 ...
 <p className="text-slate-900 font-semibold mb-1">Você ainda não criou nenhum simulado</p>
 <p className="text-slate-500 text-sm">Crie o primeiro simulado pra sua turma testar os conhecimentos.</p>
 </div>
)}
 </div>
 </div>
