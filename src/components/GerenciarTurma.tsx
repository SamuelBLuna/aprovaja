...
export default function GerenciarTurma({ turma, onFechar, onAtualizada, onExcluida }: GerenciarTurmaProps) {
  ...
  return (
    <div className="bg-white border border-ink/[0.07] rounded-xl shadow-card p-6 mb-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={onFechar} className="text-ink/40 hover:text-ink transition-colors"><X className="w-4 h-4" /></button>
        </div>
      </div>

      {/* nome */}
      <div>
        <label className="block text-xs text-ink/50 mb-1.5">Nome da turma</label>
        <div className="flex gap-2">
          <input value={nome} onChange={(e) => setNome(e.target.value)} onBlur={salvarNome}
            className="flex-1 border border-ink/15 rounded-lg px-3 py-2.5 text-sm focus:border-gold outline-none transition-colors" />
      ...
      {/* alunos */}
      <div>
        <p className="text-xs text-ink/50 mb-2 flex items-center gap-1.5"><Users className="w-3.5 h-3.5" /> Alunos matriculados ({alunos.length})</p>
        {carregandoAlunos ? (
          <p className="text-ink/40 text-sm">Carregando…</p>
        ) : alunos.length === 0 ? (
          <p className="text-ink/40 text-sm">Nenhum aluno usou o código <strong className="font-mono">{turma.codigo_turma}</strong> ainda.</p>
        ) : (
          <div className="border border-ink/[0.07] rounded-lg divide-y divide-ink/[0.06] max-h-64 overflow-y-auto">
            {alunos.map((a) => (
