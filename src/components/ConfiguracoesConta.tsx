...
export default function ConfiguracoesConta() {
 ...
 <form onSubmit={salvarNome} className="bg-white border border-slate-200 rounded-xl p-5">
 ...
 <h2 className="text-base font-semibold text-slate-900">Nome</h2>
 </div>
 <input value={nome} onChange={(e) => setNome(e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:border-gold outline-none transition-colors mb-3" />
 {msgNome && <p className="text-sm text-acerto mb-3">{msgNome}</p>}
 <button disabled={salvandoNome} className="bg-ink text-white px-4 py-2.5 rounded-lg text-sm hover:bg-ink-light transition-colors disabled:opacity-50">
 {salvandoNome ? 'Salvando…' : 'Salvar'}
 </button>
 </form>

 <form onSubmit={salvarSenha} className="bg-white border border-slate-200 rounded-xl p-5">
 <div className="flex items-center gap-2 mb-3">
 <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center"><KeyRound className="w-4 h-4 text-slate-500" /></div>
 ...
 className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:border-gold outline-none transition-colors" />
 ...
 <button disabled={salvandoSenha} className="bg-ink text-white px-4 py-2.5 rounded-lg text-sm hover:bg-ink-light transition-colors disabled:opacity-50">
 {salvandoSenha ? 'Salvando…' : 'Atualizar senha'}
 </button>
 </form>

 <div className="bg-white border border-erro/15 rounded-xl p-5">
 ...
 <p className="text-slate-500 text-sm mb-3">Sua conta será desativada, mas poderá ser recuperada fazendo login novamente antes da remoção definitiva.</p>
