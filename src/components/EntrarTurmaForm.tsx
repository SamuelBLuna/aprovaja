...
export default function EntrarTurmaForm({ onSuccess, titulo }: EntrarTurmaFormProps) {
 ...
 {titulo && <p className="text-sm text-slate-500">{titulo}</p>}
 <div className="flex gap-2">
 <input
 value={codigo}
 onChange={(e) => setCodigo(e.target.value)}
 placeholder="Código da turma (ex: TRT2024)"
 className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm uppercase focus:border-gold"
 />
 <button disabled={loading} className="bg-ink text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-ink-light transition-colors disabled:opacity-50 shrink-0">
