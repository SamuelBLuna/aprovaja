      ...
      {notificacoes.map((n) => (
        <div key={n.id} className="bg-gold/10 border border-gold/30 rounded-lg px-4 py-3 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-ink/70 mt-0.5">{n.mensagem}</p>
          </div>
          <button onClick={() => dispensar(n.id)} className="text-ink/40 hover:text-ink shrink-0 text-lg leading-none" aria-label="Dispensar">
            ×
          </button>
        </div>
      ))}
